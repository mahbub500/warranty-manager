...
class Helper {
	...
	public static function get_posts( $args = [], $show_heading = false, $show_cached = false ) {
		...
		// use cache
		if( true === $show_cached && ( $cached_posts = wp_cache_get( "wms_{$_args['post_type']}", 'wms' ) ) ) {
			$posts = $cached_posts;
		}
		...
		// don't use cache
			...
			wp_cache_add( "wms_{$_args['post_type']}", $posts, 'wms', 3600 );
		}

		$posts = $show_heading ? [ '' => sprintf( __( '- Choose a %s -', 'wms' ), $_args['post_type'] ) ] + $posts : $posts;

		return apply_filters( 'wms_get_posts', $posts, $_args );
	}
	 ...
	 *
	 * It'll look into /warranty-managment directory of your active theme
	 * first. if not found, default template will be used.
	 * can be overwriten with warranty-managment_template_overwrite_dir hook
	 *
	...
	public static function get_template( $slug, $base = 'views', $args = null ) {
		...
		$overwrite_template_dir = apply_filters( 'wms_template_overwrite_dir', get_stylesheet_directory() . '/warranty-managment/', $slug, $base, $args );
		
		// default template directory
		$plugin_template_dir = dirname( WARRANTY_MANAGMENT ) . "/{$base}/";
		...
		elseif ( file_exists( $plugin_template_path ) ) {
			...
			return __( 'Template not found!', 'warranty-managment' );
		}
	}
}